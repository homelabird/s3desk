...
- `vendor-ui` (antd + rc-*): about `~194 kB gzip`
...
| P2 | `@rc-component/form` (useForm, Field) | Formerly: `frontend/src/pages/profiles/ProfileModal.tsx`, many modals/hooks | **Done:** Replace antd `Form` usage with native `<form>` + controlled inputs, and patch antd to avoid pulling `@rc-component/form` via shared internals (ConfigProvider validate messages, Radio group helper, Form context provider) | Removes `@rc-component/form` from `vendor-ui`; less form magic to debug |
