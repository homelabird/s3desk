...
| P1 | `@rc-component/tree` (`Tree.js`) | `frontend/src/pages/objects/ObjectsTreeView.tsx`, `frontend/src/components/LocalPathBrowseModal.tsx` | Replace `Tree.DirectoryTree` with a minimal tree/list UI (only the features we use) | Shrinks `vendor-ui` and reduces complex tree behaviors/bugs |
