...
- [ ] Presigned 미지원 프로필: **staging fallback** 동작 확인
- [ ] CORS 제한 환경: 업로드 실패 시 에러 메시지 확인

## 4) 다운로드 (Direct → Proxy fallback)

- [ ] Direct download URL 성공
- [ ] CORS 제한 환경: direct 실패 후 **proxy fallback** 확인
...
- [ ] Live E2E (presigned 미지원 + CORS 제한 시나리오 포함)
...
E2E_S3_REGION=us-east-1 npx playwright test \
  tests/api-crud.spec.ts tests/jobs-live-flow.spec.ts tests/objects-live-flow.spec.ts tests/docs-smoke.spec.ts

# 모바일 스모크
cd frontend
npx playwright test tests/mobile-smoke.spec.ts --project=mobile-iphone --project=mobile-android
```
