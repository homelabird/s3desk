...
test('jobs create, cancel, retry flow', async ({ page }) => {
	...
	await expect(page.locator('tr', { hasText: 'job-created' })).toBeVisible()

	const runningRow = page.locator('tr', { hasText: 'job-running' })
	await runningRow.getByRole('button', { name: 'Cancel' }).click()
	await expect(runningRow.getByText('canceled')).toBeVisible()

	const failedRow = page.locator('tr', { hasText: 'job-failed' })
	await failedRow.getByRole('button', { name: 'Retry' }).click()
	await expect(page.locator('tr', { hasText: 'job-retry-1' })).toBeVisible()
})
