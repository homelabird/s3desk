...
function createHarness(args: { prefix?: string; favoritesOpenDetails?: boolean } = {}) {
	...
	const rendered = renderHook(() => {
		const [apiToken, setApiToken] = useState('token-a')
...
describe('useObjectsSelectionEffects', () => {
	...
	it('clears the current selection when the api token changes', () => {
		const { result } = createHarness({ prefix: 'docs/', favoritesOpenDetails: true })
		...
		expect(result.current.selectedKeys.size).toBe(0)
		expect(result.current.lastSelectedObjectKey).toBeNull()
	})

	it('drops a pending favorite selection when the api token changes', () => {
		const { result, navigateToLocation } = createHarness({ prefix: '', favoritesOpenDetails: true })
