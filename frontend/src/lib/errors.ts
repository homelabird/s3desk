...
export function getRecoveryHint(err: unknown): string | undefined {
	...
	if (err instanceof APIError) {
		...
		if (err.code === 'conflict' && err.message.toLowerCase().includes('active jobs')) {
			return 'Cancel or wait for the profile\'s active jobs to finish, then try deleting the profile again.'
		}
