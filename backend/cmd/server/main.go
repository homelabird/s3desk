...
func main() {
	...
	flag.StringVar(&cfg.APIToken, "api-token", getEnv("API_TOKEN", ""), "optional local API token (X-Api-Token); remote non-loopback binds require a non-placeholder token of at least 32 bytes")
	flag.BoolVar(&cfg.AllowRemote, "allow-remote", false, "allow non-local bind and accept private remote clients (requires API_TOKEN and ENCRYPTION_KEY when using a non-loopback addr)")
	flag.StringVar(&cfg.EncryptionKey, "encryption-key", getEnv("ENCRYPTION_KEY", ""), "base64 key to encrypt profile credentials at rest; required for remote non-loopback binds")
