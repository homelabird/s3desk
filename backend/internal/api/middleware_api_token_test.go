...
func TestValidCredentialsSurviveLockedNewClientIP(t *testing.T) {
	for _, address := range []string{"192.0.2.8:3000", "198.51.100.9:4000", "[2001:db8::9]:5000"} {
		t.Run(address, func(t *testing.T) {
			api := &server{cfg: config.Config{APIToken: "correct-token"}, authLimit: newAuthFailureLimiter(1, time.Minute, time.Minute)}
			r := httptest.NewRequest(http.MethodGet, "/api/v1/buckets", nil)
			r.RemoteAddr = address
			key := authLimiterClientKey(r)
			now := time.Now()
			api.authLimit.recordFailure(key, now)
			r.Header.Set("X-Api-Token", "correct-token")
			if !newAPITokenAuthService(api).authorize(httptest.NewRecorder(), r) {
				t.Fatal("valid credentials blocked by the client's failed-attempt lockout")
			}
			// A valid user must not erase the other client's attack history.
			r.Header.Set("X-Api-Token", "bad-token")
			rr := httptest.NewRecorder()
			if newAPITokenAuthService(api).authorize(rr, r) || rr.Code != http.StatusTooManyRequests {
				t.Fatalf("invalid credentials bypassed the active lockout: status=%d", rr.Code)
			}
		})
	}
}
