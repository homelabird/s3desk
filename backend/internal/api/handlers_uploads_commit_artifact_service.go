...
func (svc uploadCommitArtifactService) buildBasePayload(uploadID string, us store.UploadSession, req uploadCommitRequest) map[string]any {
...
func (svc uploadCommitArtifactService) buildFromRequest(uploadID string, us store.UploadSession, req uploadCommitRequest) uploadCommitArtifacts {
	payload := svc.buildBasePayload(uploadID, us, req)
	...
	return uploadCommitArtifacts{
		...
		progress:     svc.buildProgress(req),
	}
}

func (svc uploadCommitArtifactService) buildProgress(req uploadCommitRequest) *models.JobProgress {
...
func (svc uploadCommitArtifactService) buildFromVerified(uploadID string, us store.UploadSession, req uploadCommitRequest, verified []verifiedUploadObject, includeTotals bool, itemsTruncated bool) uploadCommitArtifacts {
	payload := svc.buildBasePayload(uploadID, us, req)
	...
	return uploadCommitArtifacts{
		payload:      payload,
		indexEntries: indexEntries,
		progress:     svc.buildVerifiedProgress(len(verified), totalBytes, includeTotals),
	}
}

func (svc uploadCommitArtifactService) buildVerifiedProgress(totalFiles int, totalBytes int64, includeTotals bool) *models.JobProgress {
	if !includeTotals {
		return nil
	}
