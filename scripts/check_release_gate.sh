...
E2E_RUNNER_DOCKERFILE="${ROOT}/e2e/runner/Dockerfile"
K8S_RBAC_MANIFEST="${ROOT}/k8s/gitlab-runner-namespace-admin.yaml"
HELM_SMOKE_SCRIPT="${ROOT}/scripts/helm_k8s_smoke.sh"
HELM_CI_VALUES="${ROOT}/charts/s3desk/ci-values.yaml"
VERCEL_CONFIG="${ROOT}/frontend/vercel.json"
LIGHTHOUSE_CONFIG="${ROOT}/lighthouserc.js"
LIGHTHOUSE_AUTH_SCRIPT="${ROOT}/scripts/lighthouse_puppeteer_auth.js"
...
check_shell_script_syntax
python3 "${ROOT}/scripts/check_demo_seaweedfs_test.py"
python3 "${ROOT}/scripts/check_seaweedfs_unified_test.py"
...
require_text "${LATEST_CHANGELOG_SECTION}" "Azure immutability and legal-hold editing require ARM credentials in addition to storage credentials." "Azure immutability and legal-hold known limitation"
...
require_no_text "${DEMO_COMPOSE}" "docker.io/minio/" "MinIO image dependency in SeaweedFS demo"
require_no_text "${DEMO_COMPOSE}" "seaweedfs:latest" "floating SeaweedFS demo server image"
require_text "${DEMO_COMPOSE}" '${SEAWEEDFS_BIND_HOST:-${S3DESK_BIND_HOST:-127.0.0.1}}:${SEAWEEDFS_API_PORT:-8333}:8333' "demo SeaweedFS API bind following the dashboard bind"
require_text "${DEMO_COMPOSE}" 'ALLOWED_HOSTS: "${DEMO_ALLOWED_HOSTS:-127.0.0.1,localhost,::1},${DEMO_PUBLIC_HOST:-127.0.0.1},s3desk"' "demo browser and seed host allow-list derivation"
require_text "${DEMO_COMPOSE}" 'EXTERNAL_BASE_URL: "${DEMO_EXTERNAL_BASE_URL:-http://${DEMO_PUBLIC_HOST:-127.0.0.1}:${S3DESK_PORT:-8080}}"' "demo external URL derivation"
require_text "${ROOT}/scripts/compose.sh" 'require_explicit_demo_public_host' "explicit demo public host validation"
require_text "${ROOT}/scripts/compose.sh" 'DEMO_PUBLIC_HOST must be an IP address or hostname without a scheme, port, or path' "demo public host format validation"
require_text "${ROOT}/scripts/compose.sh" '*) export S3DESK_BIND_HOST=0.0.0.0 ;;' "non-local demo bind derivation"
...
require_text "${GITLAB_CI}" "release-images.sha256" "release image artifact checksum"
require_text "${GITLAB_CI}" "--digestfile release-postgres.digest" "published postgres image digest capture"
require_text "${GITLAB_CI}" "--format cyclonedx" "release image SBOM generation"
...
require_text "${K8S_RBAC_MANIFEST}" "s3desk.homelabird.com/helm-smoke" "smoke namespace admission label"
require_text "${K8S_RBAC_MANIFEST}" "validationActions: [\"Deny\"]" "enforcing smoke RoleBinding admission binding"
require_text "${K8S_RBAC_MANIFEST}" "      - serviceaccounts" "Helm smoke ServiceAccount permissions"
require_text "${HELM_SMOKE_SCRIPT}" '--set-string "server.encryptionKey=${ENCRYPTION_KEY}"' "Helm smoke encryption key injection"
require_text "${HELM_SMOKE_SCRIPT}" 'head -c 32 /dev/urandom' "Helm smoke generated encryption key"
require_text "${HELM_CI_VALUES}" 'encryptionKey:' "Helm CI encryption key fixture"
require_text "${VERCEL_CONFIG}" 'X-Frame-Options' "Vercel anti-clickjacking header"
require_text "${VERCEL_CONFIG}" "frame-ancestors 'none'" "Vercel frame-ancestors policy"
require_text "${LIGHTHOUSE_CONFIG}" "S3DESK_URL is required" "Lighthouse explicit target guard"
require_text "${LIGHTHOUSE_AUTH_SCRIPT}" "S3DESK_LH_ALLOW_TOKEN_STORAGE" "Lighthouse token-storage opt-in"
require_text "${LIGHTHOUSE_AUTH_SCRIPT}" "outside the configured S3DESK_URL origin" "Lighthouse origin guard"
...
require_text "${RELEASE_GATE}" "reports a candidate identity blocker when the candidate cannot be resolved or does not resolve to the checked \`--head\`" "release readiness candidate identity documentation"
...
require_text "${RELEASE_GATE}" "Go \`1.25.13\`" "Go toolchain release documentation"
...
require_text "${RELEASE_GATE}" "GitLab release-builder images are literal digest-pinned image references" "GitLab image pin release documentation"
require_text "${GITLAB_CI}" 'CI_COMMIT_REF_PROTECTED == "true"' "protected GitLab release/deploy ref guard"
require_text "${GITLAB_CI}" "deployment_tier: production" "protected production deployment environment tier"
...
require_text "${RELEASE_GATE}" "\`go_race\` runs targeted \`go test -race ./internal/api ./internal/jobs\`" "GitLab Go race quality gate documentation"
...
require_text "${TESTING_DOC}" "checks that the candidate resolves to the requested \`--head\`" "release readiness candidate identity testing documentation"
...
require_text "${TESTING_DOC}" "It exits non-zero while the candidate is unresolved, live evidence is still missing, or the candidate does not match the checked head" "release readiness testing blocker warning"
...
require_text "${TESTING_DOC}" "GitLab release-builder images must remain digest-pinned" "GitLab image pin testing documentation"
...
require_text "${LICENSE_AUDIT_WORKFLOW}" "Detect runtime license change scope" "license audit runtime scope step"
require_text "${LICENSE_AUDIT_WORKFLOW}" "backend/**/*.go" "license audit Go source scope"
require_text "${LICENSE_AUDIT_WORKFLOW}" "frontend/package-lock.json" "license audit npm dependency scope"
require_text "${LICENSE_AUDIT_WORKFLOW}" "Runtime license inputs are unchanged" "license audit required no-op path"
...
require_text "${FRONTEND_E2E_WORKFLOW}" "workflow wiring is unchanged" "frontend e2e workflow-lint not-applicable wording"
...
require_text "${RELEASE_GATE_WORKFLOW}" "./scripts/check.sh ci" "release gate main command"
require_no_text "${RELEASE_GATE_WORKFLOW}" "Install Playwright Chromium for release-gate browser smoke" "duplicate release gate Playwright install"
require_no_text "${RELEASE_GATE_WORKFLOW}" "npm run test:e2e:smoke" "duplicate release gate browser smoke"
require_text "${RELEASE_GATE_WORKFLOW}" "Core Mock E2E" "release gate required browser owner summary"
...
require_text "${GITLAB_CI}" "go_race:" "GitLab Go race job"
require_text "${GITLAB_CI}" "go test -race ./internal/api ./internal/jobs" "GitLab targeted Go race command"
